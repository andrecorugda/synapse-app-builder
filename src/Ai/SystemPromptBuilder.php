...
final class SystemPromptBuilder
{
    ...
    private const NODE_HINTS = [
        ...
        'result' => 'Return page actions to the browser (fire LAST, after the logic nodes). config: { actions:[ {type:"notify|alert|modal|redirect|logout|setHtml|addClass|removeClass", ...} ] }. `notify` toasts {message,level}; `modal` opens/closes a Modal block by {target:"#id"} (optionally showing a {partial}); `redirect` navigates {url}. To write app state, use the Set Variable node (it updates $store.app live) — NOT a result action.',
        'call_flow' => 'Run another saved flow as a sub-step, sharing this flow\'s vars/input context. config: { flow:"<flow slug>", args:{}, output:"<var>" }. The target must be a flow you define in THIS plan or that already exists; a flow may never call itself.',
    ];
    ...
    private function rules(): string
    {
        ...
        - SINGLE ENTRY POINT: every flow begins with EXACTLY ONE `trigger` node — that is the flow's entry. Set the definition's `start` to the trigger's id, and the trigger's `next` points to the first logic node (e.g. `"start":"trigger","nodes":{"trigger":{"type":"trigger","next":["guard"]}, ...}`). Never start a flow directly on a condition/record/transaction, and never emit two triggers — to run different processes, branch from a single trigger with `condition` nodes (different `next_true`/`next_false` → different results).
