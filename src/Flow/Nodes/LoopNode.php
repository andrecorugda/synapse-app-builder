...
class LoopNode implements FlowNodeHandler, ProvidesNodeDefinition
{
    ...
    public function definition(): CapabilityDefinition
    {
        return new CapabilityDefinition(
            ...
            icon: 'arrow-path',
            inputs: [
                new CapabilityInput('over', 'Array to loop over', 'expression', required: true, help: 'A context path resolving to an array, e.g. vars.items or input.lines.'),
                new CapabilityInput('item_var', 'Item variable', 'string', default: 'item', help: 'The body reads the current element as {{ vars.<item_var> }}.'),
                new CapabilityInput('index_var', 'Index variable', 'string', default: 'index', help: 'Zero-based position of the current element.'),
                new CapabilityInput('max_iterations', 'Max iterations', 'number', help: 'Safety cap on passes (default '.self::ITERATION_CEILING.').'),
                new CapabilityInput('output', 'Output variable', 'string', interpolated: false, help: 'Stores {count: <iterations>} under this var when set.'),
            ],
