    ...
    public function definition(): CapabilityDefinition
    {
        return new CapabilityDefinition(
            ...
            inputs: [
                ...
                new CapabilityInput('id', 'Record ID', 'expression', help: 'Record ID for get / update / delete (interpolated).', showIf: ['operation' => ['get', 'update', 'delete']]),
                new CapabilityInput('filter', 'Filter', 'json', help: 'Filter object for list, e.g. {"status": {"eq": "open"}} (interpolated).', showIf: ['operation' => ['list']]),
                new CapabilityInput('search', 'Search', 'string', help: 'Full-text search term for list (interpolated).', showIf: ['operation' => ['list']]),
                new CapabilityInput('sort', 'Sort', 'string', help: 'Sort expression for list, e.g. "name" or "-created_at" (interpolated).', showIf: ['operation' => ['list']]),
                new CapabilityInput('per_page', 'Per page', 'number', help: 'Page size for list (optional).', showIf: ['operation' => ['list']]),
                new CapabilityInput('page', 'Page', 'number', default: 1, help: 'Page number for list (default 1).', showIf: ['operation' => ['list']]),
                new CapabilityInput('data', 'Data', 'json', help: 'Field values for create / update (interpolated).', showIf: ['operation' => ['create', 'update']]),
