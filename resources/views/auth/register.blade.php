...
<head>
    ...
    <title>Create account &middot; {{ config('app.name', 'Synapse') }}</title>
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .links { text-align: center; margin-top: 1.1rem; color: #94a3b8; font-size: 0.85rem; }
...
<body>
    <div class="card">
        ...
        <form method="POST" action="{{ url('/'.$loginPath.'/register') }}">
            @csrf
            <div class="field">
                <label for="name">Name</label>
                <input id="name" type="text" name="name" value="{{ old('name') }}" autocomplete="name" autofocus required>
            </div>
            <div class="field">
        ...
        <div class="links">
            Already have an account? <a href="{{ url('/'.$loginPath) }}">Sign in</a>
        </div>
